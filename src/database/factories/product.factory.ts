...
function makeProduct(
  categories: CategoryPgEntity[],
  locations: LocationPgEntity[],
): ProductPgEntity {
...
export async function createProduct(dataSource: DataSource, count: number) {
  ...
  const products: ProductPgEntity[] = Array.from({ length: count }).map(() =>
    makeProduct(categoriesAvailable, locationsAvailable),
  );
