...
export class Product1744915919425 implements MigrationInterface {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        const category_fk = table?.foreignKeys.find(fk => fk.columnNames.includes('category_id'));
        if (category_fk) {
            await queryRunner.dropForeignKey('product', category_fk);
        }

        const location_fk = table?.foreignKeys.find(fk => fk.columnNames.includes('location_id'));
        if (location_fk) {
